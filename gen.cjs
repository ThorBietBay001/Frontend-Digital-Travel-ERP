        ...
        case 'object':
            ...
            if (schema.properties) {
                const props = Object.entries(schema.properties).map(([key, propSchema]) => {
                    ...
                    return '  ' + safeKey + (isRequired ? '' : '?') + ': ' + mapType(propSchema) + ';';
                }).join('\n');
                return '{\n' + props + '\n}';
            }
...
function buildMethod(operationId, method, url, op) {
    ...
    if (op.parameters) {
        op.parameters.forEach(p => {
            if (p.in === 'path') {
                const safeName = isValidIdentifier(p.name) ? p.name : p.name.replace(/[^a-zA-Z0-9_$]/g, '');
    ...
    if (returnType && returnType.startsWith('ApiResponse')) extractData = true;

    let paramSig = params.join(', ');
    const urlStr = url.includes('{') ? "`" + formatUrl(url) + "`" : "'" + url + "'";
